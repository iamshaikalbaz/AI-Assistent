...
const SignUp = () => {
  ...
  const { serverUrl,userData,setUserData } = useContext(userDataContext);
    const navigate = useNavigate(); // ✅ Now navigate will work
  ...
  navigate("/customize"); // ✅ only on success
} catch (error) {
  console.log(error);
  setUserData(null);
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center px-8 py-4 bg-black/30 backdrop-blur-sm text-white">
        ...
        <nav className="space-x-6">
          ...
          <Link
            ...
            onClick={() => setShowFormSignUp(true)}
          >
            Sign Up
          </Link>
        </nav>
      ...
      {showFormSignUp && (
        <div className="absolute inset-0 flex items-center justify-center px-4">
          <div className="bg-black/1 backdrop-blur-xl p-8 rounded-2xl shadow-2xl w-full max-w-md border border-white/40 relative">
            {/* Close Button */}
            <button
              onClick={() => setShowFormSignUp(false)}
            ...
            <form className="flex flex-col space-y-4" onSubmit={handleSignUp}>
              ...
              <div>
                ...
                {/* Live error message */}
                {passwordError && (
                  <p className="text-white text-sm mt-2 text-center">{passwordError}</p>
                )}
              </div>
